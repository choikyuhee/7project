...
<body>
	...
	<p id="a1">
		<c:if test ="${member.u_id == article.u_id || member.u_admin == 1 }">
			<a id="a1" class="btn btn-primary" href="${contextPath }/board/freeModiView.do?no=${article.no}">글수정</a>
			<a id="a1" class="btn btn-primary" href="${contextPath }/board/freeDelete.do?no=${article.no}">글삭제</a>
		</c:if>
		<a id="a2" class="btn btn-primary" href="${contextPath }/board/free.do?num=1">글목록</a>
	</p>
		...
		<c:choose>
			<c:when test="${isLogOn == true && member != null }">
				<form action="${contextPath }/board/insertFreeReply.do" method="post">
					<p>
